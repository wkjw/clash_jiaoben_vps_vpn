...
def format_host_for_uri(host):
    """在URI中使用的主机名，IPv6需要加方括号"""
    if is_ipv6_address(host) and not host.startswith('['):
        return f'[{host}]'
    return host

def format_host_port(host, port):
    """格式化主机和端口，自动处理IPv6格式"""
    return f"{format_host_for_uri(host)}:{port}"

def parse_listen_port(value, default=443):
    """从listen配置解析端口号，兼容:443或[::]:443等格式"""
    if value is None:
        return default
    if isinstance(value, int):
        return value
    listen = str(value).strip()
    if not listen:
        return default
    # 去掉方括号包裹的IPv6地址
...
def set_nginx_permissions(web_dir):
    ...
    try:
        ...
        # 设置目录和文件权限
        subprocess.run(['chown', '-R', f'{nginx_user}:{nginx_user}', web_dir], check=True)
        subprocess.run(['chmod', '-R', '755', web_dir], check=True)
        subprocess.run(['find', web_dir, '-type', 'f', '-exec', 'chmod', '644', '{}', ';'], check=True)
...
http {{
    ...
    server {{
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.conf') as tmp:
            ...
            tmp.flush()
            subprocess.run(['cp', tmp.name, '/etc/nginx/nginx.conf'], check=True)
            os.unlink(tmp.name)
        
        subprocess.run(['rm', '-f', '/etc/nginx/conf.d/*.conf'], check=True)
        ...
        if test_result.returncode != 0:
            ...
            subprocess.run(['cp', '/etc/nginx/nginx.conf.backup', '/etc/nginx/nginx.conf'], check=True)
 ...
 <body>
     ...
     <section class="features" id="services">
         <div class="container">
             ...
             <div class="features-grid">
                 ...
                 <div class="feature">
                     <div class="feature-icon">[MANAGED]</div>
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.html') as tmp:
            ...
            tmp.flush()
            subprocess.run(['cp', tmp.name, f"{web_dir}/index.html"], check=True)
            os.unlink(tmp.name)
...
"""
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.txt') as tmp:
            ...
            tmp.flush()
            subprocess.run(['cp', tmp.name, f"{web_dir}/robots.txt"], check=True)
            os.unlink(tmp.name)
  ...
  <url>
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.xml') as tmp:
            ...
            tmp.flush()
            subprocess.run(['cp', tmp.name, f"{web_dir}/sitemap.xml"], check=True)
            os.unlink(tmp.name)
    ...
    try:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.ico') as tmp:
                ...
                tmp.flush()
                subprocess.run(['cp', tmp.name, f"{web_dir}/favicon.ico"], check=True)
                os.unlink(tmp.name)
    except:
...
<body>
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.html') as tmp:
            ...
            tmp.flush()
            subprocess.run(['cp', tmp.name, f"{web_dir}/about.html"], check=True)
            os.unlink(tmp.name)
...
<body>
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.html') as tmp:
            ...
            tmp.flush()
            subprocess.run(['cp', tmp.name, f"{web_dir}/404.html"], check=True)
            os.unlink(tmp.name)
...
def create_config(base_dir, port, password, cert_path, key_path, domain, enable_web_masquerade=True, custom_web_dir=None, enable_port_hopping=False, obfs_password=None, enable_http3_masquerade=False):
    ...
    if enable_port_hopping:
        # Hysteria2服务器端只监听单个端口，端口跳跃通过ip6tables REDIRECT实现
...
def delete_hysteria2():
    ...
    try:
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, 'r') as f:
                    config = json.load(f)
                listen_value = config.get('listen', '[::]:443')
                listen_port = parse_listen_port(listen_value, default=443)
        ...
        # 保存ip6tables规则
        try:
            subprocess.run(['ip6tables-save'], check=True, capture_output=True)
            subprocess.run(['netfilter-persistent', 'save'], check=False, capture_output=True)
        except:
            try:
                subprocess.run(['service', 'ip6tables', 'save'], check=False, capture_output=True)
            except:
                pass
    ...
    try:
        ...
        for conf_file in nginx_conf_files:
            if os.path.exists(conf_file):
                try:
                    subprocess.run(['rm', '-f', conf_file], check=True)
        ...
        for web_dir in nginx_web_dirs:
            if os.path.exists(web_dir):
                ...
                if os.path.exists(backup_file):
                    try:
                        subprocess.run(['cp', backup_file, f"{web_dir}/index.html"], check=True)
    ...
    try:
        ...
        for service_file in service_files:
            if os.path.exists(service_file):
                try:
                    subprocess.run(['systemctl', 'stop', 'hysteria2'], check=False)
                    subprocess.run(['systemctl', 'disable', 'hysteria2'], check=False)
...
支持的防墙功能:
1. 端口跳跃 (Port Hopping): 动态切换端口，防止端口封锁
2. Salamander混淆: 加密流量特征，防DPI深度包检测
3. HTTP/3伪装: 流量看起来像正常HTTP/3网站访问
4. Web页面伪装: nginx显示正常网站页面
...
推荐配置:
1. 最佳推荐: --simple (端口跳跃+混淆+nginx Web伪装)
2. 完整功能: --one-click (一键部署所有功能)
3. 速度优先: 基础安装
4. 稳定优先: --port-hopping
5. 隐蔽优先: --obfs-password + --http3-masquerade
""")
...
def setup_dual_port_masquerade(base_dir, domain, web_dir, cert_path, key_path):
    ...
    if not has_nginx:
        ...
        try:
            ...
            if shutil.which('apt'):
                ...
                subprocess.run(['yum', 'install', '-y', 'epel-release'], check=True)  # EPEL for nginx
                ...
                subprocess.run(['pacman', '-S', '--noconfirm', 'nginx'], check=True)
            elif shutil.which('zypper'):
                safe_print("使用Zypper包管理器安装nginx...")
                subprocess.run(['zypper', 'install', '-y', 'nginx'], check=True)
            else:
    ...
    if not nginx_web_dir:
        ...
        try:
            subprocess.run(['mkdir', '-p', nginx_web_dir], check=True)
    ...
    try:
        ...
        try:
            if os.path.exists(f"{nginx_web_dir}/index.html"):
                subprocess.run(['cp', f'{nginx_web_dir}/index.html', f'{nginx_web_dir}/index.html.backup'], check=True)
        ...
        if os.path.exists(web_dir):
            ...
            try:
                subprocess.run(['find', web_dir, '-type', 'f', '-exec', 'cp', '{}', nginx_web_dir, ';'], check=True)
                ...
                for file in os.listdir(web_dir):
                    ...
                    if os.path.isfile(src_file):
                        subprocess.run(['cp', src_file, nginx_web_dir], check=True)
            safe_print(f"伪装文件已复制到: {nginx_web_dir}")
        else:
...
server {{
    ...
    add_header X-Content-Type-Options nosniff always;
        ...
        # 删除可能存在的旧配置
        subprocess.run(['rm', '-f', f'/etc/nginx/conf.d/{domain}.conf'], check=False)
        subprocess.run(['rm', '-f', f'/etc/nginx/sites-enabled/{domain}'], check=False)
        subprocess.run(['rm', '-f', f'/etc/nginx/sites-available/{domain}'], check=False)
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.conf') as tmp:
            tmp.write(ssl_conf)
            tmp.flush()
            subprocess.run(['cp', tmp.name, ssl_conf_file], check=True)
            os.unlink(tmp.name)
...
def main():
    ...
    if args.command == 'del':
        ...
        if not os.path.exists(config_path):
            safe_print("配置文件不存在，请先安装 Hysteria2")
            sys.exit(1)
        with open(config_path, 'r') as f:
            config = json.load(f)
...
""")
        ...
        config_link = f"hysteria2://{urllib.parse.quote(password)}@{host_port}?insecure={insecure_param}&sni={server_address}"
...
优化带宽配置 (1000mbps)  
降低日志级别
{'nginx Web伪装已配置' if nginx_success else 'nginx未配置 (建议运行: python3 hy2.py setup-nginx)'}
{'真实域名证书' if use_real_cert else '自签名证书 (建议使用真实域名证书)'}
...
def deploy_hysteria2_complete(server_address, port=443, password="ISDdwk@ASI47!F#WE", enable_real_cert=False, domain=None, email="admin@example.com", port_range=None, enable_bbr=False):
    ...
    if port_range:
        # 准备下载链接
        download_links = {
            "v2rayN多端口订阅 (推荐)": f"http://{uri_host}:8080/v2rayn-subscription.txt",
            "多端口配置明文查看": f"http://{uri_host}:8080/multi-port-links.txt",
            "Clash多端口配置": f"http://{uri_host}:8080/clash.yaml", 
            "官方客户端配置": f"http://{uri_host}:8080/hysteria-official.yaml",
            "JSON配置 (完整功能)": f"http://{uri_host}:8080/hysteria2.json"
        }
...
# 1. 服务器端ip6tables REDIRECT: 将{port_start}-{port_end}流量转发到{port}
...
"""
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.conf') as tmp:
                ...
                tmp.flush()
                subprocess.run(['cp', tmp.name, sysctl_file], check=True)
                os.unlink(tmp.name)
        ...
        # 立即启用BBR
        try:
            subprocess.run(['sysctl', '-w', 'net.core.default_qdisc=fq'], check=True)
            subprocess.run(['sysctl', '-w', 'net.ipv4.tcp_congestion_control=bbr'], check=True)
            safe_print("BBR已立即生效")
        except Exception as e:
...
if __name__ == "__main__":
    PORT = 8080
    try:
        with IPv6TCPServer(("::", PORT), ConfigHandler) as httpd:
...
'''
        ...
        # 开放防火墙端口（8080用于配置下载）
        subprocess.run(['ip6tables', '-A', 'INPUT', '-p', 'tcp', '--dport', '8080', '-j', 'ACCEPT'], check=False)
        ...
        try:
            ...
            try:
                ...
                pass
            result = sock.connect_ex(('::1', 8080, 0, 0))
            sock.close()
            if result == 0:
...
def show_final_summary(server_address, port, port_range, password, obfs_password, config_link, enable_port_hopping=False, download_links=None, num_ports=None):
    ...
    host_port = format_host_port(server_address, port)
    host_443 = format_host_port(server_address, 443)
    ...
    hysteria_443_url = f"hysteria2://{urllib.parse.quote(password)}@{host_443}?insecure=1&sni={server_address}&obfs=salamander&obfs-password={urllib.parse.quote(obfs_password)}#Hysteria2-443"
    ...
    if port_range and '-' in str(port_range):
        ...
        for i, random_port in enumerate(random_ports, 1):
            random_host_port = format_host_port(server_address, random_port)
            random_url = f"hysteria2://{urllib.parse.quote(password)}@{random_host_port}?insecure=1&sni={server_address}&obfs=salamander&obfs-password={urllib.parse.quote(obfs_password)}#V2Ray-{random_port}-{i:02d}"
...
load_config() {{
    ...
    URI_HOST=$(echo "$CONFIG" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('uri_host', data['server_address']))" 2>/dev/null || echo "$SERVER_ADDRESS")
    PORT=$(echo "$CONFIG" | python3 -c "import sys, json; print(json.load(sys.stdin)['port'])" 2>/dev/null || echo "N/A")
    HOST_PORT=$(echo "$CONFIG" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('host_port', '{{}}:{{}}'.format(data['server_address'], data['port'])))" 2>/dev/null || echo "$SERVER_ADDRESS:$PORT")
...
show_node_info() {{
    ...
    if [ -n "$RANDOM_PORTS" ]; then
        ...
        for port in $RANDOM_PORTS; do
            url="hysteria2://$(python3 -c \"import urllib.parse; print(urllib.parse.quote('$PASSWORD'))\")@${{URI_HOST}}:$port?insecure=1&sni=$SERVER_ADDRESS&obfs=salamander&obfs-password=$(python3 -c \"import urllib.parse; print(urllib.parse.quote('$OBFS_PASSWORD'))\")#V2Ray-$port"
            echo "$url"
            if [ -z "$URLS" ]; then
                URLS="$url"
            else
...
show_config_info() {{
    ...
    echo ""
    echo "配置文件下载地址:"
    echo "v2rayN多端口订阅: http://${{URI_HOST}}:8080/v2rayn-subscription.txt"
    echo "多端口配置明文: http://${{URI_HOST}}:8080/multi-port-links.txt"
    echo "Clash多端口配置: http://${{URI_HOST}}:8080/clash.yaml"
    echo "官方客户端配置: http://${{URI_HOST}}:8080/hysteria2.json"
...
# 删除服务
delete_service() {{
    echo "确认要删除Hysteria2服务吗？这将删除所有配置和文件！"
    echo "输入 'yes' 确认删除，其他任意键取消:"
    read -r confirm
    
    if [ "$confirm" = "yes" ]; then
        echo "正在删除Hysteria2服务..."
...
def generate_multi_port_subscription(server_address, password, obfs_password, port_start, port_end, base_dir, num_configs=100):
    ...
    for i, port in enumerate(selected_ports, 1):
        ...
        # 生成hysteria2链接
        hysteria2_host_port = format_host_port(server_address, port)
        hysteria2_url = f"hysteria2://{encoded_password}@{hysteria2_host_port}?insecure=1&sni={server_address}&obfs=salamander&obfs-password={encoded_obfs_password}#{encoded_node_name}"
